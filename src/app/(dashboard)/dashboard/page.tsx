...
export default function DashboardPage() {
  ...
  const percentBadgeVariant = deal.percentPaid < 20 ? 'error' : deal.percentPaid < 40 ? 'warning' : 'info';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-5">
          <p className="text-xs font-medium text-[#6B7280] uppercase tracking-wide mb-1">Total RTR</p>
          <p className="text-2xl font-bold text-[#16325C]">{formatCurrency(deal.totalRTR)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-5">
          <p className="text-xs font-medium text-[#6B7280] uppercase tracking-wide mb-1">Balance RTR</p>
          <p className="text-2xl font-bold text-[#16325C]">{formatCurrency(deal.balanceRTR)}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-5">
          <p className="text-xs font-medium text-[#6B7280] uppercase tracking-wide mb-1">Percent Paid</p>
          <div className="flex items-center gap-2 mt-1">
            <p className="text-2xl font-bold text-[#16325C]">{deal.percentPaid}%</p>
            <Badge variant={percentBadgeVariant}>{deal.percentPaid}% paid</Badge>
          </div>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-5">
          <p className="text-xs font-medium text-[#6B7280] uppercase tracking-wide mb-1">Next Payment</p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-semibold text-[#16325C]">{deal.percentPaid}% Paid Off</h2>
      ...
      {deal.renewalSubmitted ? (
        <div className="rounded-xl border border-green-200 bg-green-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-green-800 text-sm">Renewal request submitted</p>
            <p className="text-green-700 text-sm mt-0.5">Our team will review your request and contact you within 2-3 business days.</p>
          </div>
        </div>
      ) : deal.percentPaid >= 40 ? (
        <div className="rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 flex items-center justify-between gap-3">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-[#0057FF] flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <p className="font-medium text-blue-800 text-sm">You&apos;re eligible for renewal!</p>
              <p className="text-blue-700 text-sm mt-0.5">You&apos;ve paid {deal.percentPaid}% of your balance. Apply for a renewal now.</p>
            </div>
          </div>
          <Link href="/renewal" className="text-sm font-semibold text-[#0057FF] hover:text-[#004AE0] whitespace-nowrap transition-colors">
            Renew Now &rarr;
          </Link>
        </div>
      ) : (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-amber-800 text-sm">Renewal not yet available</p>
            <p className="text-amber-700 text-sm mt-0.5">Reach 40% to unlock renewal. You&apos;re currently at {deal.percentPaid}%.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-6 flex flex-col">
          <h2 className="text-base font-semibold text-[#16325C] mb-1">Make a Payment</h2>
          <p className="text-sm text-[#6B7280] mb-4">Submit a one-time ACH payment toward your balance.</p>
          ...
          {nextPayment && (
            <div className="bg-[#F5F7FA] border border-[#E5E7EB] rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
              <div>
                <p className="text-xs text-[#6B7280]">Next scheduled</p>
                <p className="text-sm font-semibold text-[#16325C]">{formatCurrency(nextPayment.amount)} · {formatDate(nextPayment.date)}</p>
              </div>
              <div className="flex items-center gap-1.5 text-xs text-[#6B7280]">
                <svg className="w-4 h-4 text-[#0057FF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-[#16325C]">Recent Payments</h2>
            <Link href="/payments" className="text-xs text-[#0057FF] hover:text-[#004AE0] font-medium transition-colors">
              View all
            </Link>
          </div>
            ...
            {recentPayments.map((p) => (
              <div
                ...
                className={`flex items-center justify-between py-2 border-b border-[#E5E7EB] last:border-0 ${
                  p.status === 'bounced' || p.status === 'failed' ? 'bg-red-50 -mx-2 px-2 rounded-lg' : ''
                }`}
              >
                <div>
                  <p className="text-sm font-medium text-[#16325C]">{formatDate(p.date)}</p>
                  <p className="text-xs text-[#6B7280]">{p.method} · {p.referenceId}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold text-[#16325C]">{formatCurrency(p.amount)}</span>
